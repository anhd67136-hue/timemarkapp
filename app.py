  ...
  "shortcuts": [
    {
      "name": "Chụp ảnh mới",
      "short_name": "Chụp",
      "description": "Chụp ảnh và ghép overlay",
      "url": "/",
...
def render_page(title: str, body_html: str, user_row=None) -> str:
    ...
    <meta name="apple-mobile-web-app-title" content="TimeMark">
...
function fitCanvasToImage(w,h){ 
  // Giữ nguyên kích thước ảnh gốc, không scale
  cv.width = w; 
  cv.height = h; 
  maskCanvas.width = w; 
  maskCanvas.height = h; 
...
function drawLeftCluster(ctx, W, H, reservedRight, timeVal, dateText, dowText){ 
  // Xác định BASE dựa trên diện tích và tỷ lệ ảnh
  const area = W * H;
  const aspectRatio = W / H;
  
  // BASE scaling: Dựa trên diện tích ảnh
  let BASE;
  if (area < 500000) { // < 720p
    BASE = Math.sqrt(area) * 0.85;
  } else if (area < 2000000) { // 720p - 1080p
    BASE = Math.sqrt(area) * 0.75;
  } else { // >= 1080p
    BASE = Math.sqrt(area) * 0.68;
  }
  
  // Điều chỉnh theo aspect ratio
  if (aspectRatio < 0.6) { // Ảnh dọc (portrait)
    BASE = BASE * 0.9;
  } else if (aspectRatio > 1.5) { // Ảnh ngang (landscape)
    BASE = BASE * 1.1;
  }
  
  const leftMaxWidth = Math.round(W * 0.58);
  const leftX = clamp(Math.round(W * 0.025), 10, 50);
  const bottomPad = clamp(Math.round(H * 0.035), 12, 70);
  const rightLimit = Math.min(leftX + leftMaxWidth, W - reservedRight);
  const maxWidth = Math.max(160, rightLimit - leftX);
  const shadowBlur = clamp(Math.round(BASE * 0.004), 2, 8);
  
  // Địa chỉ
  const addr1 = "268B Võ Nguyên Giáp, Bắc Mỹ Phú, Ngũ";
  const addr2 = "Hành Sơn, Đà Nẵng 550000";
  let addrFont = clamp(Math.round(BASE * 0.042), 12, 36);
  const addrTpl = (s)=>`400 ${s}px "Inter", -apple-system, sans-serif`;
  addrFont = fitFontToWidth(ctx, addr1, addrTpl, addrFont, 10, maxWidth);
  const addrLineH = Math.round(addrFont * 1.25);
  const addrBlockH = addrLineH * 2;
  const gapMetaToAddr = clamp(Math.round(BASE * 0.032), 10, 40);
  
  // Giờ (time)
  let timeFont = clamp(Math.round(BASE * 0.095), 28, 90);
  const timeTpl = (s)=>`700 ${s}px "Inter", -apple-system, sans-serif`;
  const minMetaW = Math.round(maxWidth * 0.35);
  const maxTimeW = Math.max(80, maxWidth - minMetaW);
  timeFont = fitFontToWidth(ctx, timeVal, timeTpl, timeFont, 18, maxTimeW);
  const timeScaleY = 1.5;
  
  // Ngày & thứ (meta)
  let metaFont = clamp(Math.round(timeFont * 0.40), 11, 38);
  const metaTpl = (s)=>`500 ${s}px "Inter", -apple-system, sans-serif`;
  ...
  // Khoảng cách X
  const gapX = clamp(Math.round(BASE * 0.018), 8, 24);
...
function drawWatermark(ctx, W, H){ 
  ...
  // BASE scaling dựa trên diện tích
  let BASE;
  if (area < 500000) { // < 720p
    BASE = Math.sqrt(area) * 0.85;
  } else if (area < 2000000) { // 720p - 1080p  
  ...
  // Điều chỉnh theo aspect ratio
  if (aspectRatio < 0.6) { // Portrait
    BASE = BASE * 0.9;
  } else if (aspectRatio > 1.5) { // Landscape
    BASE = BASE * 1.1;
  }
  
  const padR = clamp(Math.round(W * 0.022), 10, 42);
  const padB = clamp(Math.round(H * 0.032), 12, 60);
  ...
  let wmFont = clamp(Math.round(BASE * 0.055), 16, 48);
  let subFont = clamp(Math.round(wmFont * 0.55), 10, 26);
  
  const wmTpl = (s)=>`700 ${s}px "Inter", -apple-system, sans-serif`;
  const subTpl = (s)=>`600 ${s}px "Inter", -apple-system, sans-serif`;
  
  const maxWmWidth = Math.round(W * 0.35);
  wmFont = fitFontToWidth(ctx, fullText, wmTpl, wmFont, 12, maxWmWidth);
  subFont = clamp(Math.round(wmFont * 0.55), 10, 26);
  ...
  const yTop = yBottom - Math.round(subFont * 1.22);
  ctx.textAlign = "left";
  ctx.font = wmTpl(wmFont);
  ...
  return padR + Math.round(wmWidth) + clamp(Math.round(W*0.032), 12, 42);
}
